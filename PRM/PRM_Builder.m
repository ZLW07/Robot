...
while true
    if(iteration_times >= sampling_points) % if exceed the sample times
        break;
    end
    ...
    if(map(ceil(rand_node(2)), ceil(rand_node(1))) ~= 2) % if the random node on the free space
        ...
        counter = 0;
        for i = 1:list_size(1)  % traverse the whole node_list to find adjacent nodes
            dis = distance(rand_node, node_list(i).coordinate); 
            if(dis < step_length_limit && ObstacleCheck(rand_node, node_list(i).coordinate, map, 0.01) == 0) % obstacle check and step length limit 
                add2list_flag = 1;
                if(counter == 0)
                    node_temp = Node(rand_node, 'W');
                    counter = counter + 1;
                end
                node_temp = node_temp.addAdjacentNode(node_list(i).coordinate, dis, i);
                node_list(i) = node_list(i).addAdjacentNode(node_temp.coordinate, dis, list_size(1)+1);
            end
        end
...
if(show_graph)
    for i = 1:size(node_list, 1)
        adjacent_node_num = size(node_list(i).adjacent_coordinate, 1);
        for k = 1:adjacent_node_num
            plot([node_list(i).coordinate(1), node_list(i).adjacent_coordinate(k,1)], [node_list(i).coordinate(2), node_list(i).adjacent_coordinate(k,2)], 'b');
        end
    end
end
NodeList = node_list;
